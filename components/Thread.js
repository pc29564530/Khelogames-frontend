...
const Thread = () => {
    ...
    const handleLikes = async (id) => {
      try {
        ...
        const userCount = await axiosInstance.get(`http://192.168.0.100:8080/checkLikeByUser/${id}`, {headers});
        console.log("Usercount: ", userCount.data)
        if(userCount.data == 0) {
          const response = await axiosInstance.post(`http://192.168.0.100:8080/createLikeThread/${id}`,null, {headers} );
          if(response.status === 200) {
            try {
              const updatedLikeCount = await axiosInstance.get(`http://192.168.0.100:8080/countLike/${id}`,null,{headers});
              const updateLikeData = {
                like_count: updatedLikeCount.data,
                id: id
              }

              const newLikeCount = await axiosInstance.put(`http://192.168.0.100:8080/update_like`, updateLikeData, {headers});
    ...
    const fetchData = async () => {
      try {
        const authToken = await AsyncStorage.getItem('AccessToken');
        const response = await axiosInstance.get('http://192.168.0.100:8080/all_threads', {
          headers: {
            'Authorization': `Bearer ${authToken}`,
            'Content-Type': 'application/json',
          },
        ...
        if(item === null){
          ...
          const threadUser = item.map(async (item,index) => {
            const profileResponse = await axiosInstance.get(`http://192.168.0.100:8080/getProfile/${item.username}`);
    ...
    const handleUser = async (username) => {
      try {
        ...
        if(username === undefined || username === null) {
          const response = await axiosInstance.get(`http://192.168.0.100:8080/user/${user}`);
          navigation.navigate('Profile', { username: response.data.username });
        } else {
          const response = await axiosInstance.get(`http://192.168.0.100:8080/user/${username}`);
          navigation.navigate('Profile', { username: response.data.username });
        }
