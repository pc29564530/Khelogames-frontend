    ...
    const handleAddSubstitution = async () => {
        ...
        try {
            ...
            if (isMountedRef.current) {
                ...
                console.error("Unable to do substitution:", err?.response?.data?.error);
            }
        } finally {
            if (isMountedRef.current) {
                setLoading(false);
                        ...
                        <View style={tailwind`px-4 py-3`}>
                            ...
                            <View style={tailwind`flex-row items-center`}>
                                <Text style={{ color: '#64748b', fontSize: 11 }}>
                                    {teamName} - {selectedHalf === 'first_half' ? '1st Half' : '2nd Half'} - {selectedMinute}{extraTime > 0 ? `+${extraTime}` : ''}'
                                </Text>
                            </View>
                        </View>
                    </View>
