...
const TournamentBadmintonMatch = ({ tournament, AsyncStorage, axiosInstance, BASE_URL, parentScrollY, collapsedHeader}) => {
    ...
    const fetchTournamentMatchs = async () => {
        try {
            ...
            //     )
            // }
            console.log("Line no 54: ", item.data)
                            ...
                            <View key={index} style={{ backgroundColor: '#0f172a' }}>
                                {Object.keys(stage.group_stage).length > 0 &&
                                    Object.entries(stage.group_stage).map(([stageName, matchs]) => (
                                        <View key={stageName}>
                                            <Text style={{ color: '#f1f5f9', fontSize: 17, marginBottom: 8 }}>
                                                {stageName}
                                            </Text>

                                            {matchs?.length > 0 && matchs?.map((item, ind) => (
...
export default TournamentBadmintonMatch;
